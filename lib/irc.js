...
exports.newServer = function (obj) {
    ...
    server.addListener('message#', function(from, to , message) {
        ...
        if(message.toString().substring(0, 6) == "!stop ") {
            var java_server = message.toString().substring(6);
            if(!files.servers[java_server]){
                server.say(to, 'No Server by the name of ' + java_server);
            }
                ...
                if (files.allowall) {
                    server.say(to, 'Stopping ' + java_server);
                    java.javaStop(files.servers[java_server], function(response, error){
                       server.say(to, response);
                    });
                }
                else {
                    authorized(from, function (data) {
        ...
        if(message.toString().substring(0, 5) == '!tail'){
            if(message.toString() == '!tail'){
                server.say(to, 'Will send you console output via pm in irc. Usage: !tail <start|stop> <server>')
            }
            else{
                var state = message.toString().substring(6).split(' ')[0];
                var java_server = message.toString().substring(6).split(' ')[1];
                if(state == 'start' || state == 'stop'){
                    if(files.servers[java_server]){
                        if(files.allowall){
                            if(state == 'start'){
                                java.javaTail(files.servers[java_server], from, function(response, error){
                                   server.say(to, response);
                                });
                                return;
                            }
                            if(state == 'stop'){
                                java.javaTailStop(files.servers[java_server], from, function(response, error){
                                   server.say(to, response);
                                });
                            }
                        }
                        ...
                        server.say(to, 'No server found by that name. Usage: !tail <start|stop> <server>');
                    }
                }
                else{
                    server.say(to, 'Will send you console output via pm in irc. Usage: !tail <start|stop> <server>');
        ...
        if(message.toString() == '!help'){
            if(files.allowcommands){
                ...
                if(string == ""){
                    server.say(to, 'Bot Commands are: !summon, !kill, !restart, !addcmd, !editcmd, !delcmd, !allowadd, !allowrm, !tail');
                }
                server.say(to, 'Bot Commands are: !summon, !kill, !restart, !addcmd, !editcmd, !delcmd, !allowadd, !allowrm, !tail, ' + string);
            }
            else{
                server.say(to, 'Bot Commands are: !summon, !kill, !restart, !allowadd, !allowrm, !tail');
            }
